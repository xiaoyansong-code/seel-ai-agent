 ...
 * Self-contained so the module's look is independent from the existing
 * shadcn-derived kit. Strict with the design DNA in the PRD.
 */
...
/* ── Surface ───────────────────────────────────────────────
   A plain white panel with 1px border and 6px radius.
   No box-shadow (per PRD: cards are flat 1px borders). */
...
/* ── Button ───────────────────────────────────────────────
   Variants are neutral — no indigo tinting inside the Sales Agent module.
   Primary == near-black (#111), radius 4px. */
...
export const SAButton = forwardRef<HTMLButtonElement, SABtnProps>(
  function SAButton(
    ...
    const base =
      "inline-flex items-center justify-center gap-1.5 whitespace-nowrap font-medium rounded-[4px] transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-neutral-900/10 disabled:pointer-events-none disabled:opacity-40 tabular-nums";
    const sizes = {
  ...
  function SAInput({ className, ...props }, ref) {
    return (
      <input
        ref={ref}
        className={cn(
          "h-8 w-full rounded-[4px] border border-neutral-300 bg-white px-2.5 text-[13px] text-neutral-900 placeholder:text-neutral-400 outline-none focus:border-neutral-900 focus:ring-1 focus:ring-neutral-900/10 disabled:bg-neutral-50 disabled:text-neutral-400",
          className,
        )}
        {...props}
      />
  ...
  function SASelect({ className, children, ...props }, ref) {
    return (
      <select
        ref={ref}
        className={cn(
          "h-8 rounded-[4px] border border-neutral-300 bg-white pl-2.5 pr-7 text-[13px] text-neutral-900 outline-none focus:border-neutral-900 focus:ring-1 focus:ring-neutral-900/10 disabled:bg-neutral-50 disabled:text-neutral-400 appearance-none",
...
export function Chip({ children, onRemove, className }: ChipProps) {
  return (
    <span
      className={cn(
        "inline-flex items-center gap-1 h-6 pl-2 pr-1 rounded-[4px] bg-neutral-100 text-neutral-800 text-[12px]",
        !onRemove && "pr-2",
        className,
      )}
    >
...
export function Callout({ tone, title, children, className }: CalloutProps) {
  ...
  return (
    <div
      className={cn(
        "text-[12px] leading-relaxed px-3 py-2 rounded-[4px] border",
        styles[tone],
        className,
      )}
    >
...
export function Modal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div
        className={cn(
          "relative w-[calc(100%-32px)] bg-white rounded-md shadow-[0_10px_32px_-12px_rgba(0,0,0,0.25)] border border-neutral-200 flex flex-col max-h-[calc(100vh-64px)]",
          width,
        )}
        role="dialog"
        aria-modal="true"
      >
        ...
        {footer && (
          <div className="flex items-center justify-end gap-2 px-4 py-3 border-t border-neutral-200 bg-neutral-50/50">
            {footer}
          </div>
        )}
      </div>
...
export function Segmented<T extends string>({
  ...
  return (
    <div
      className={cn(
        "inline-flex rounded-[4px] bg-neutral-100 p-0.5",
        h,
      )}
    >
      {options.map((opt) => {
        ...
        return (
          <button
            ...
            className={cn(
              "px-3 rounded-[3px] font-medium transition-colors",
              text,
              active
