 ...
 * - Overview: Merged Performance buttons into one "View in Performance", removed Conversational Management
 * - Configuration: Skills default to globally enabled, Live Chat channel uses Agent Name + Escalation Email
...
interface Skill {
  ...
  enabled: boolean;
  isDefault?: boolean;
  globalEnabled?: boolean; // Whether this skill is globally enabled in Playbook
...
/* All skills — globalEnabled reflects Playbook-level defaults */
const allSkills: Skill[] = [
  { id: "s1", name: "Post-purchase Claims", desc: "Refund requests, damaged items, missing orders", enabled: true, globalEnabled: true },
  { id: "s2", name: "Where Is My Order (WISMO)", desc: "Order tracking, shipping updates, delivery inquiries", enabled: true, globalEnabled: true },
  { id: "s3", name: "Order Changes", desc: "Cancellations, address changes, item swaps", enabled: false, globalEnabled: false },
  { id: "s4", name: "Returns & Exchanges", desc: "Returns, labels, exchanges", enabled: false, globalEnabled: false },
  { id: "s5", name: "Subscription Management", desc: "Pause, resume, upgrade, cancel subscriptions", enabled: false, globalEnabled: false },
  { id: "s6", name: "Product Inquiry", desc: "Product questions, recommendations", enabled: false, globalEnabled: false },
];

const skillIconMap: Record<string, { icon: typeof Package; color: string }> = {
  s1: { icon: Shield, color: "bg-blue-50 text-blue-600" },
  s2: { icon: Package, color: "bg-primary/10 text-primary" },
  s3: { icon: ShoppingCart, color: "bg-amber-50 text-amber-600" },
  s4: { icon: RefreshCw, color: "bg-purple-50 text-purple-600" },
  s5: { icon: Globe, color: "bg-cyan-50 text-cyan-600" },
  s6: { icon: HelpCircle, color: "bg-gray-100 text-gray-600" },
};
...
const agentsDb: Record<string, AgentData> = {
  "rc-chat": {
    ...
    skills: [
      { id: "s1", name: "Post-purchase Claims", desc: "Refund requests", enabled: true, globalEnabled: true, conversations: 276, successRate: 94.2 },
      { id: "s2", name: "Where Is My Order (WISMO)", desc: "Order tracking", enabled: true, globalEnabled: true, conversations: 342, successRate: 97.1 },
      { id: "s3", name: "Order Changes", desc: "Cancellations", enabled: true, globalEnabled: false, conversations: 154, successRate: 88.5 },
    ],
...
function SettingUpView({ agent }: { agent: AgentData }) {
  ...
  const isChat = agent.channel.type === "chat";
  const [agentDisplayName, setAgentDisplayName] = useState(isChat ? "Seel Support" : "");
  const [escalationEmail, setEscalationEmail] = useState(isChat ? "support@company.com" : "");
    ...
    <div className="space-y-8">
      ...
      <section>
        ...
        <div className="space-y-5">
            ...
            {!zdConnected ? (
              ...
              <Card className="border-primary/20 bg-primary/[0.02]">
                <CardContent className="p-4 flex items-center gap-3">
                  ...
                  <div className="flex-1"><p className="text-sm font-medium">{agent.channel.integration}</p><p className="text-[11px] text-muted-foreground">{zdSubdomain}.zendesk.com · Connected</p></div>
          ...
          {/* Webhook / Trigger */}
          <div>
            <h3 className="text-xs font-medium mb-2">Trigger Configuration</h3>
            <Card>
                ...
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-muted-foreground">Receives events from {agent.channel.integration}</p>
                    </div>
                  </div>
                  <Badge variant="outline" className={cn("text-[9px]", webhookStatus === "connected" ? "text-primary border-primary/20" : "text-muted-foreground")}>
                    {webhookStatus === "connected" ? "Connected" : webhookStatus === "testing" ? "Testing..." : "Waiting"}
                  </Badge>
                </div>
                <div className="flex items-center gap-2">
                  <code className="flex-1 text-[10px] bg-muted px-3 py-2 rounded font-mono truncate">https://api.seel.ai/webhooks/{agent.channel.type}/{agent.slug.toLowerCase().replace(/\s/g, "-")}</code>
                  <Button variant="outline" size="sm" className="text-xs h-7" onClick={() => { setWebhookStatus("testing"); setTimeout(() => { setWebhookStatus("connected"); toast.success("Webhook connected"); }, 1500); }}>
                    {webhookStatus === "testing" ? <Loader2 className="w-3 h-3 animate-spin" /> : "Test"}
                  </Button>
                  <Button variant="ghost" size="sm" className="text-xs h-7" onClick={() => setTriggerGuideOpen(true)}>Guide</Button>
                </div>
              </CardContent>
            </Card>
          </div>
      ...
      <section>
        <div className="flex items-center justify-between mb-3">
          <div>
            ...
            <p className="text-[11px] text-muted-foreground mt-0.5">Globally enabled skills are on by default</p>
          </div>
          <Button variant="outline" size="sm" className="text-xs gap-1 h-7" onClick={() => setShowAddSkill(true)}><Plus className="w-3 h-3" /> Add Skill</Button>
        </div>
          ...
          {skills.map(skill => {
            ...
            return (
              <div key={skill.id} className={cn("flex items-center gap-3 p-3 rounded-lg border transition-all", skill.enabled ? "border-border bg-card" : "border-border/50 bg-muted/10 opacity-60")}>
                ...
                <Switch checked={skill.enabled} onCheckedChange={() => setSkills(prev => prev.map(s => s.id === skill.id ? { ...s, enabled: !s.enabled } : s))} />
              </div>
            );
          })}
        </div>
      ...
      {/* ═══ Channel Settings ═══ */}
      <section>
        <h2 className="text-base font-semibold mb-3">Channel Settings</h2>
        <Card>
          <CardContent className="p-4 space-y-4">
            <div className="flex items-center gap-3">
              <div className={cn("w-9 h-9 rounded-lg flex items-center justify-center", isChat ? "bg-primary/10" : "bg-blue-50")}>
                {isChat ? <MessageCircle className="w-4 h-4 text-primary" /> : <Mail className="w-4 h-4 text-blue-500" />}
              </div>
              <div>
              ...
              {isChat ? (
                <>
                  <div>
                    ...
                    <Input value={agentDisplayName} onChange={e => setAgentDisplayName(e.target.value)} placeholder="e.g., Seel Support" className="mt-1" />
                    <p className="text-[10px] text-muted-foreground mt-1">The name shown to customers in the chat widget</p>
                  </div>
                  <div>
                    <Label className="text-xs">Welcome Message</Label>
                    <Textarea defaultValue="Hi there! I'm your support assistant. How can I help you today?" rows={2} className="mt-1" />
                  </div>
                  <div>
                    <Label className="text-xs">Escalation Email</Label>
                    <Input value={escalationEmail} onChange={e => setEscalationEmail(e.target.value)} placeholder="support@company.com" className="mt-1" />
                    <p className="text-[10px] text-muted-foreground mt-1">Escalated conversations will be forwarded to this email address</p>
                  </div>
                </>
              ) : (
                <>
      ...
      <Dialog open={triggerGuideOpen} onOpenChange={setTriggerGuideOpen}>
        <DialogContent className="max-w-[500px]">
          <DialogHeader>
            <DialogTitle className="text-sm">Webhook Setup Guide</DialogTitle>
            <DialogDescription className="text-xs">Follow these steps to connect your {agent.channel.integration} to Seel AI</DialogDescription>
          </DialogHeader>
          <div className="space-y-3 mt-2">
            {["Go to your platform admin panel → Webhooks", "Create a new webhook with the URL above", "Select 'New Ticket' and 'New Message' events", "Save and click 'Test' to verify the connection"].map((step, i) => (
              <div key={i} className="flex items-start gap-2.5">
                <div className="w-5 h-5 rounded-full bg-primary/10 flex items-center justify-center shrink-0 mt-0.5"><span className="text-[10px] font-bold text-primary">{i + 1}</span></div>
    ...
    <div className="space-y-6">
      <Card className="border-blue-200 bg-blue-50/30">
        <CardContent className="p-5 text-center space-y-3">
          ...
          <div>
            <h2 className="text-sm font-semibold">Ready to Test</h2>
            <p className="text-xs text-muted-foreground mt-1 max-w-[400px] mx-auto">
              Your agent is configured and ready for testing. Run test conversations to verify it handles customer inquiries correctly before going live.
            </p>
          </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <Card>
          <CardHeader className="pb-1 px-4 pt-3"><CardTitle className="text-xs font-semibold">Skills Enabled</CardTitle></CardHeader>
        ...
        <Card>
          <CardHeader className="pb-1 px-4 pt-3"><CardTitle className="text-xs font-semibold">Channel</CardTitle></CardHeader>
          <CardContent className="px-4 pb-3">
            <div className="flex items-center gap-2">
              {agent.channel.type === "email" ? <Mail className="w-3.5 h-3.5 text-blue-500" /> : <MessageCircle className="w-3.5 h-3.5 text-primary" />}
              <span className="text-[11px]">{agent.channel.label} via {agent.channel.integration}</span>
            </div>
          </CardContent>
        </Card>
      </div>
...
/* ═══════════════════════════════════════════ */
/* ── Configuration Tab                       */
/* Skills default to globally enabled         */
/* Live Chat: Agent Name + Escalation Email   */
/* Guardrails: Coming Soon                    */
/* ═══════════════════════════════════════════ */
function ConfigurationTab({ agent }: { agent: AgentData }) {
  // Skills: merge agent skills with all skills, defaulting globalEnabled ones to enabled
  const [skills, setSkills] = useState<Skill[]>(() => {
    const agentSkillIds = new Set(agent.skills.map(s => s.id));
    const merged: Skill[] = agent.skills.map(s => ({ ...s }));
    // Add globally enabled skills that aren't already in agent's list
    allSkills.forEach(gs => {
      if (!agentSkillIds.has(gs.id) && gs.globalEnabled) {
        merged.push({ ...gs, enabled: true });
      }
    });
    return merged;
  });
  ...
  // Channel config state — different for chat vs email
  const isChat = agent.channel.type === "chat";
  const [agentDisplayName, setAgentDisplayName] = useState(isChat ? "Seel Support" : "");
  const [escalationEmail, setEscalationEmail] = useState(isChat ? "support@company.com" : "");
  ...
  // Count diffs from global
  const diffCount = skills.filter(s => {
    const global = allSkills.find(g => g.id === s.id);
    return global && global.globalEnabled !== s.enabled;
  }).length;
  ...
  const handleSyncWithGlobal = () => {
    setSkills(prev => prev.map(s => {
      const global = allSkills.find(g => g.id === s.id);
      if (global) return { ...s, enabled: global.globalEnabled || false };
      return s;
    }));
    markChanged();
    toast("Skills synced with global configuration");
  };
            ...
            <div className="space-y-3 pt-3 border-t">
              {isChat ? (
                <>
                  <div>
                    <Label className="text-xs">Agent Display Name</Label>
                    <Input value={agentDisplayName} onChange={e => { setAgentDisplayName(e.target.value); markChanged(); }} placeholder="e.g., Seel Support" className="mt-1" />
                    <p className="text-[10px] text-muted-foreground mt-1">The name shown to customers in the chat widget</p>
                  </div>
                  <div>
                    <Label className="text-xs">Welcome Message</Label>
                  ...
                  <div>
                    <Label className="text-xs">Escalation Email</Label>
                    <Input value={escalationEmail} onChange={e => { setEscalationEmail(e.target.value); markChanged(); }} placeholder="support@company.com" className="mt-1" />
                    <p className="text-[10px] text-muted-foreground mt-1">Escalated live chat conversations will be forwarded to this email</p>
                  </div>
                </>
              ) : (
                <>
...
/* ═══════════════════════════════════════════ */
/* ── Simulator Panel                         */
/* Batch Test marked V1.1 Coming Soon         */
/* ═══════════════════════════════════════════ */
function SimulatorPanel({ agentName }: { agentName: string }) {
  ...
  return (
    <div className="space-y-3">
      {/* Mode tabs — Single active, Batch marked V1.1 */}
      <div className="flex items-center gap-2">
        <Button variant="default" size="sm" className="text-xs gap-1 h-7">
          <MessageSquare className="w-3 h-3" /> Single Test
        </Button>
        <Button variant="outline" size="sm" className="text-xs gap-1 h-7 opacity-60 cursor-not-allowed" disabled>
          <Zap className="w-3 h-3" /> Batch Test
          <Badge variant="secondary" className="text-[8px] px-1 py-0 ml-1">V1.1</Badge>
        </Button>
      </div>
...
function TestPanel({ agentName }: { agentName: string }) {
  ...
  const [msgs, setMsgs] = useState<{ role: "customer" | "agent"; text: string }[]>([]);
  const [running, setRunning] = useState(false);

  const handleSend = () => {
    if (!input.trim()) return;
    setMsgs(prev => [...prev, { role: "customer", text: input }]);
    setInput("");
    setRunning(true);
    setTimeout(() => {
      setMsgs(prev => [...prev, { role: "agent", text: "I found your order! It's currently in transit with FedEx. Expected delivery is in 2 business days." }]);
      setRunning(false);
    }, 1200);
  };
    ...
    <div className="flex-1 flex flex-col">
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        {msgs.length === 0 && (
          <div className="h-full flex flex-col items-center justify-center text-center">
            <Bot className="w-8 h-8 text-muted-foreground/30 mb-2" />
            <p className="text-xs text-muted-foreground">Test how {agentName} responds to customer messages</p>
          </div>
        )}
        {msgs.map((msg, i) => (
          <div key={i} className={cn("flex", msg.role === "customer" ? "justify-end" : "justify-start")}>
            <div className={cn("max-w-[80%] rounded-xl px-3 py-2 text-xs", msg.role === "customer" ? "bg-muted" : "bg-primary/10 border border-primary/15")}>
              {msg.text}
            </div>
          </div>
        ))}
      ...
      <div className="flex gap-2 p-3 border-t">
        ...
        <Button size="sm" onClick={handleSend} className="h-8 px-3" disabled={running || !input.trim()}>
          <Send className="w-3.5 h-3.5" />
        </Button>
      </div>
    </div>
